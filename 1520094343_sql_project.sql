...
SELECT
  count(name) as num_no_fee_facilities
FROM Facilities
WHERE membercost = 0
...
SELECT
  ...
  CASE WHEN monthlymaintenance > 100 THEN 'expensive'
    ELSE 'cheap' END AS 'costly_maintenance?'
FROM Facilities
...
SELECT
  ...
  CASE WHEN Members.firstname = 'GUEST' THEN Members.firstname
    ELSE CONCAT(Members.firstname, ' ', Members.surname) END AS mem_name
FROM(
    SELECT
...
JOIN Members
ON Members.memid = booking.mem_id
ORDER BY 2, 1
...
SELECT
  ...
  CASE WHEN m.firstname = 'GUEST' THEN m.firstname
    ELSE CONCAT(m.firstname, ' ', m.surname) END AS mem_name,
...
SELECT
  ...
  CASE WHEN m.firstname = 'GUEST' THEN m.firstname
    ELSE CONCAT(m.firstname, ' ', m.surname) END AS mem_name,
  CASE WHEN m.firstname = 'GUEST' THEN booking.guestcost*booking.slots
    ELSE booking.membercost*booking.slots END AS booking_cost
FROM(
    SELECT
      b.memid as memid,
      b.slots as slots,
...
LEFT JOIN Members m
ON m.memid = booking.memid
WHERE (CASE WHEN m.memid = 0 THEN booking.guestcost*booking.slots
    ELSE booking.membercost*booking.slots END) > 30
ORDER BY 3 DESC
